...
class Withdrawal {
    ...
    public function create($user, $amount, $cardNumber) {
        if ($amount <= 0) {
            return false;
        }
        ...
        $stmt = $this->pdo->prepare("
            INSERT INTO withdrawal_requests (user_id, amount, card_number, reference_code, status, created_at)
            VALUES (?, ?, ?, ?, 'pending', NOW())
        ");
        $stmt->execute([$user->getId(), $amount, $cardNumber, $referenceCode]);
...
function handleMessage($pdo, $telegram, $message) {
    ...
    // دستور مخصوص ادمین برای ساخت کوپن مستقیم از تلگرام: /addcoupon CODE مبلغ
    // مثال: /addcoupon WELCOME1000 1000
    if (str_starts_with($text, '/addcoupon')) {
        ...
        // $parts[0] = /addcoupon, $parts[1] = کد, $parts[2] = مبلغ
        if (count($parts) !== 3 || !preg_match('/^[A-Za-z0-9]{6,20}$/', $parts[1]) || !ctype_digit($parts[2])) {
            $telegram->sendMessage(
                "فرمت درست:\n/addcoupon CODE مبلغ\n\nمثال:\n/addcoupon WELCOME1000 1000\n\n(کد باید فقط حروف/عدد انگلیسی و ۶ تا ۲۰ کاراکتر باشد)",
                $chatId
            );
            return;
        }
        ...
        try {
            $createdCode = $coupon->create($amount, $user->getId(), $code);
            $telegram->sendMessage("✅ کوپن ساخته شد:\nکد: {$createdCode}\nمبلغ: {$amount} سکه", $chatId);
        } catch (Exception $e) {
            $telegram->sendMessage("❌ این کد قبلاً وجود دارد یا مشکلی پیش آمد. یه کد دیگه امتحان کن.", $chatId);
        }
        return;
    }

    // درخواست برداشت: /withdraw مبلغ شماره‌کارت
    // مثال: /withdraw 5000 6037991199990017
    if (str_starts_with($text, '/withdraw')) {
        $parts = preg_split('/\s+/', trim($text));

        if (count($parts) !== 3 || !ctype_digit($parts[1])) {
            $telegram->sendMessage(
                "فرمت درست:\n/withdraw مبلغ شماره‌کارت\n\nمثال:\n/withdraw 5000 6037991199990017",
                $chatId
            );
            return;
        }
        ...
        $amount = (int) $parts[1];
        $cardDigitsOnly = preg_replace('/\D/', '', $parts[2]);

        if (strlen($cardDigitsOnly) !== 16) {
            $telegram->sendMessage('❌ شماره کارت باید ۱۶ رقمی باشد.', $chatId);
            return;
        }

        $withdrawal = new Withdrawal($pdo);
        $result = $withdrawal->create($user, $amount, $cardDigitsOnly);
        ...
        // اطلاع فوری به ادمین با دکمه‌های تایید/رد
    ...
    if ($awaitingInput === 'custom_bet') {
        ...
        $amount = (int) $text;
        $tableId = create_bet_table_and_announce($pdo, $telegram, $user, $chatId, $amount);
...
function handleCallbackQuery($pdo, $telegram, $callbackQuery) {
    ...
    if (str_starts_with($data, 'join:')) {
        ...
        // قبلاً یه کپی جدا به پیوی خصوصی لیدر هم می‌فرستادیم که باعث می‌شد
        // نتیجه‌ی تاسِ لیدر تو پیوی بره ولی تاسِ حریف تو گروه - و بازی نصفه‌نیمه پخش بشه.
        $rollKeyboard = TelegramAPI::inlineKeyboard([[TelegramAPI::inlineButton('🎲 انداختن تاس', 'roll:' . $tableId)]]);
        $telegram->sendMessage('🎲 حریف پیدا شد! هر دو طرف دکمه زیر را بزنید تا تاس بیندازید:', $chatId, $rollKeyboard);
        return;
    }
    ...
    if (str_starts_with($data, 'wd_approve:') || str_starts_with($data, 'wd_reject:')) {
        ...
        if ($userTelegramId) {
            $msg = $isApprove
                ? "✅ درخواست برداشت شما (کد {$request['reference_code']}) تایید و واریز شد."
    ...
    if (str_starts_with($data, 'roll:')) {
        ...
        // بازی تمام شد - هم تو همون چت (گروه) اعلام می‌کنیم، هم جدا برای هر بازیکن تو پیوی خودش
        ...
        // اگه بازی تو خودِ پیوی یکی از این دو نفر انجام شده باشه، دوباره همون پیام رو
        // براش نفرست (چون همین الان تو $chatId گرفتش)
 ...
 */
function create_bet_table_and_announce($pdo, $telegram, $user, $chatId, $amount) {
    $game = new Game($pdo);
